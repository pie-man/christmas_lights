...
noblocks = 6
block_len = NUM_LEDS // (noblocks * 2)
blocks=[]
colours = []
for count in range(noblocks):
    blocks.append(list(a + (2 * count * block_len) for a in range(block_len)))
    colours.append(count / noblocks)

values = list(a / (block_len - 1) for a in range(block_len))

for steps in range(NUM_LEDS * 3):
    for point in range(block_len):
        count = 0
        for block in blocks:
            block[point] += 1
            block[point] = block[point]%NUM_LEDS
...
print(f"Debug: state[:10] = {new_state[:10]}")
